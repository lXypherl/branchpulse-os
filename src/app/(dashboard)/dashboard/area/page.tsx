...
async function getAreaData(userId: string, areaId: string) {
  try {
    const [area, branches, activeAudits, openIssues] = await Promise.all([
...
export default async function AreaDashboardPage() {
      ...
      <div className="min-h-screen bg-background px-6 py-8 lg:px-10">
        <div className="mx-auto max-w-lg rounded-xl border border-outline-variant/15 bg-surface-container-lowest p-8 shadow-ambient text-center">
          ...
          <p className="text-sm text-on-surface-variant">
            You don&apos;t have access to this page. This dashboard is for Area Managers only.
          </p>
        </div>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-background px-6 py-8 lg:px-10">
        <div className="mx-auto max-w-lg rounded-xl border border-outline-variant/15 bg-surface-container-lowest p-8 shadow-ambient text-center">
          ...
          <p className="text-sm text-on-surface-variant">
            Your account is not linked to an area. Please contact your administrator.
          </p>
        </div>
      </div>
    );
  ...
  const data = await getAreaData(session.id, session.areaId);
    ...
    <div className="min-h-screen bg-background px-6 py-8 lg:px-10">
      ...
      <header className="mb-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-on-surface-variant">
            {data.regionName ? `${data.regionName} \u2022 ` : ''}{data.branchCount} Branches in your area
          </p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-ambient">
          ...
          <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-secondary/10">
            <div
              ...
              style={{ width: `${Math.min((data.activeAudits / 10) * 100, 100)}%` }}
            />
          </div>
        </div>
        ...
        <div className="relative overflow-hidden rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-ambient">
          ...
          <div className="mt-4 h-1.5 w-full overflow-hidden rounded-full bg-tertiary/10">
            <div
              ...
              style={{ width: `${Math.min((data.openIssueCount / 10) * 100, 100)}%` }}
            />
          </div>
        </div>
      </section>
      ...
      <section className="mb-10">
        <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-ambient">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-on-surface">Branches in Area</h2>
      ...
      <section>
        <div className="rounded-xl border border-outline-variant/10 bg-surface-container-lowest p-6 shadow-ambient">
          <div className="mb-5 flex items-center justify-between">
            <div>
              <h2 className="text-base font-bold text-on-surface">Area Issues</h2>
              <p className="mt-0.5 text-xs text-on-surface-variant">
          ...
          <div className="space-y-3">
            ...
            {data.issues.length === 0 && (
              <div className="py-8 text-center text-sm text-on-surface-variant">
                ...
                No open issues in your area. Great work!
              </div>
            )}
          </div>
        </div>
