    ...
    <div className="max-w-[1400px] mx-auto">
      ...
      <div className="space-y-6">
        {demoCampaigns.map((campaign) => (
          <Link key={campaign.id} href={`/promos/${campaign.id}`} className="block group">
            <div className="bg-surface-container-lowest rounded-xl shadow-ambient border border-outline-variant/10 p-8 group-hover:shadow-lg group-hover:border-primary/20 transition-all">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <div className="flex items-center gap-3 mb-2">
                    <h2 className="text-xl font-bold tracking-tight text-on-surface">{campaign.name}</h2>
                    <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                      campaign.status === 'active' ? 'bg-blue-100 text-blue-700' : campaign.status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'
                ...
                <div className="text-right">
                  <p className="text-3xl font-black tracking-tighter text-on-surface">
                    {campaign.branches > 0 ? Math.round((campaign.confirmed / campaign.branches) * 100) : 0}%
                  </p>
